...
var checksum = function(room) {
  var i1 = room.indexOf('[');
  var i2 = room.indexOf(']');
  return room.substring(i1+1,i2);
}

var sectorID = function(room) {
  var i = room.indexOf('[');
  var j = i;
  while (j > 0 && room.substring(j,j+1) !== '-') {
    j--;
  }
  return room.substring(j+1,i);
}

var name = function(room) {
  var i = room.indexOf(sectorID(room));
  return room.substring(0,i-1);
}

var frequency = function(s,letter) {
  return Array.from(s).filter(function(a) { return a === letter; }).length;
}

var letters = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'];

var frequencyMap = function(s) {
  var map = {};
  for (var i=0; i<letters.length; i++) {
    map[letters[i]] = frequency(s,letters[i]);
  }
  var output = "";
...
var convertLetter = function(l,n) {
...
var decrypt = function(s,n) {
  var output = "";
  for (var i=0; i<s.length; i++) {
    if(s.substring(i,i+1) === '-') {
      output = output + " ";
    } else {
      output = output + convertLetter(s.substring(i,i+1),n);
    }
  }
  return output;
}
...
for(var i=0; i<input.length; i++) {
  //console.log(name(input[i]) + "." + sectorID(input[i]) + "." + checksum(input[i]));
  //console.log(frequencyMap(name(input[i])));
  if (frequencyMap(name(input[i])).indexOf(checksum(input[i])) === 0) {
    console.log(decrypt(name(input[i]),parseInt(sectorID(input[i]))) + ": " + sectorID(input[i]));
    sectorIDSum += parseInt(sectorID(input[i]));
  }
}
