...
var tokenize = function(s) {
  ...
  while (s.length > 0) {
    ...
    if (i === -1) {
      ...
      unbracketed.push(s.substring(0,i));
    ...
    bracketed.push(s.substring(0,i));
    s = s.substring(i+1,s.length);
  }
  return {"u":unbracketed, "b":bracketed};
};

var containsABBA = function(s) {
  if (s && s.length >= 4) {
    for (var i=0; i<s.length-3; i++) {
      var a = Array.from(s.substring(i,i+4));
      if (a[0] !== a[1] && a[0] === a[3] && a[1] === a[2]) {
        return true;
      }
    }
  }
...
var ABAstrings = function(s,bracketed) {
  var strings = [];
  if (s && s.length >= 3) {
    for (var i=0; i<s.length-2; i++) {
      var a = Array.from(s.substring(i,i+3));
      if (a[0] !== a[1] && a[0] === a[2]) {
        if(bracketed) {
            strings.push("" + a[1] + a[0]);
        } else {
            strings.push("" + a[0] + a[1]);
        }
      }
    }
  }
...
for (var i=0; i<input.length; i++) {
  var a = tokenize(input[i]);
  //console.log(input[i]);
  //console.log(a);
  //console.log(a.u.map(function(c) {return containsABBA(c);}));
  var supportsTLS = false;
  for (var j=0; j<a.u.length; j++) {
    if (containsABBA(a.u[j])) {
      supportsTLS = true;
    }
  }
  for (j=0; j<a.b.length; j++) {
    if (containsABBA(a.b[j])) {
      supportsTLS = false;
    }
  }
  ...
  var abas = [];
  for (j=0; j<a.u.length; j++) {
    abas = abas.concat(ABAstrings(a.u[j],false));
  }
  var babs = [];
  for (j=0; j<a.b.length; j++) {
    babs = babs.concat(ABAstrings(a.b[j],true));
  }

  var ssl = false;
  for (j=0; j<abas.length; j++) {
    for (var k=0; k<babs.length; k++) { 
      if(abas[j] === babs[k]) {
        ssl = true;
      }
    }
